...
console.log('[Background] ⚡ Registering tabs.onUpdated listener at TOP LEVEL...');
...
chrome.tabs.onUpdated.addListener(async (tabId, changeInfo, tab) => {
  ...
  if (!tabHandlers) {
    console.debug('[DEBUG tabs.onUpdated] tabHandlers not initialized yet, skipping');
    return;
  }
  ...
  console.debug('[DEBUG tabs.onUpdated] ⚡ FIRED! tabId:', tabId, 'attached:', attachedTabId, 'match:', tabId === attachedTabId);
  console.debug('[DEBUG tabs.onUpdated] changeInfo:', JSON.stringify(changeInfo));
  console.debug('[DEBUG tabs.onUpdated] tab.url:', tab.url);
  ...
  if (changeInfo.url) {
    console.debug('[DEBUG tabs.onUpdated] Tab', tabId, 'URL changed to:', changeInfo.url);
    console.debug('[DEBUG tabs.onUpdated] Attached tab ID:', attachedTabId);
    console.debug('[DEBUG tabs.onUpdated] Match:', tabId === attachedTabId);
  ...
  if (tabId === attachedTabId && changeInfo.url && wsConnection) {
    console.log('[Background] ✅ Attached tab navigated to:', changeInfo.url);
    console.log('[Background] Sending notification to server...');
...
console.log('[Background] ✅ tabs.onUpdated listener registered at TOP LEVEL!');
